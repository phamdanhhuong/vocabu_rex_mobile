...
class _SkillPartCardStatefulState extends State<_SkillPartCardStateful> {
  ...
  Widget build(BuildContext context) {
    // Check logic
    final isCurrentPart =
        widget.skillPart.skills?.any(
          (skill) => skill.id == widget.currentSkillId,
        ) ??
        false;
    ...
    final bool canJumpAhead =
        !isActive &&
        !isCompleted; // Tất cả parts chưa hoàn thành đều có thể học vượt

    // Màu nền header - tất cả đều unlocked
    final Color headerColor = isCompleted
        ? AppColors.macawLight.withOpacity(
            0.5,
          ) // Part đã hoàn thành - màu nhạt hơn
        ...
        child: Column(
          children: [
            ...
            Container(
              ...
              height:
                  140, // Chiều cao cố định cho phần header để căn chỉnh nhân vật
              child: Row(
                crossAxisAlignment: CrossAxisAlignment
                    .end, // Căn đáy để nhân vật đứng trên line
                children: [
                      ...
                      Expanded(
                        child: Container(
                          ...
                          child: FractionallySizedBox(
                            widthFactor:
                                (widget.skillPart.progressPercentage / 100)
                                    .clamp(0.0, 1.0),
                            child: Container(
                              decoration: BoxDecoration(
                  ...
                  if (canJumpAhead) ...[
                    ...
                    InkWell(
                      onTap: () {
                        ...
                        if (widget.skillPart.skills != null &&
                            widget.skillPart.skills!.isNotEmpty) {
                          context.read<HomeBloc>().add(
                            LoadSkillPartEvent(
                              skillPartId: widget.skillPart.id,
                            ),
                          );
