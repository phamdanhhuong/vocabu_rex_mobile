...
class PodcastController extends ChangeNotifier {
  ...
  Future<void> _setupTts() async {
    ...
    await tts.setSpeechRate(AppPreferences().isVoiceSpeedNormal ? 0.8 : 0.4);
    ...
    try {
      ...
      print(
        '🎤 Cached ${_cachedVoices?.length ?? 0} voices during initialization',
      );
    } catch (e) {
  ...
  Future<void> _playSegmentsSequentially() async {
    while (_state.currentSegmentIndex < meta.segments.length) {
      ...
      print(
        '🎵 Playing segment ${_state.currentSegmentIndex}: ${segment.transcript.substring(0, segment.transcript.length > 50 ? 50 : segment.transcript.length)}...',
      );
      ...
      if (segment.questions != null &&
          ...
          !_state.segmentsWithQuestionsShown.contains(
  ...
  Future<void> _playCurrentSegment() async {
    ...
    print(
      '� Transcript: ${segment.transcript.substring(0, segment.transcript.length > 50 ? 50 : segment.transcript.length)}...',
    );
  ...
  void handleQuestionAnswered(bool isCorrect, dynamic answer) {
    ...
    if (isCorrect) {
      ...
      Future.delayed(const Duration(milliseconds: 800), () {
        ...
        _updateState(_state.copyWith(clearFeedback: true, clearQuestion: true));
