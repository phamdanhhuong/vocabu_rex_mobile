...
class CurrencyBloc extends Bloc<CurrencyEvent, CurrencyState> {
  ...
  CurrencyBloc({required this.getCurrencyBalanceUseCase})
    : super(CurrencyInitial()) {
    on<GetCurrencyBalanceEvent>(_onGetCurrencyBalance);
  }

  Future<void> _onGetCurrencyBalance(
    GetCurrencyBalanceEvent event,
    Emitter<CurrencyState> emit,
  ) async {
    print(
      '💰 CurrencyBloc: Getting currency balance for userId: ${event.userId}',
    );
    emit(CurrencyLoading());
    try {
      final balance = await getCurrencyBalanceUseCase.call(event.userId);
      print(
        '💰 CurrencyBloc: Balance loaded - Gems: ${balance.gems}, Coins: ${balance.coins}',
      );
